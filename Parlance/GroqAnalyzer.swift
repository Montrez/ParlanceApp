...
final class GroqAnalyzer: Sendable {

    static let shared = GroqAnalyzer()
    ...
    func analyze(sentence: String, language: String, level: String, ragContext: String) async throws -> [String: Any] {
        ...
        guard httpResponse.statusCode == 200 else {
            ...
            print("⚠️ Groq API error \(httpResponse.statusCode): \(body)")
    ...
    private func normalizeResult(_ raw: [String: Any]) -> [String: Any] {
        var result: [String: Any] = [:]
        result["status"] = raw["status"] as? String ?? "Excellent"
        result["grammar_rule"] = raw["grammar_rule"] as? String ?? "Unknown"
...
enum GroqError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .httpError(let code, let body): return "Groq HTTP \(code): \(body.prefix(200))"
        case .parseError: return "Failed to parse Groq response"
        }
    }
}
