...
final class ExternalAnalyzer: Sendable {
    ...
    private func buildSystemPrompt(langName: String, ragContext: String) -> String {
        ...
        let formalRegister: String
        let informalRegister: String
        switch langName {
        case "French":
        ...
        let evaluateFocus: String
        let exampleSentenceRule: String
        if langName == "English" {
            evaluateFocus = "articles, tense aspect, conditionals, false cognates from Spanish/French, preposition calques, register (\(registerLabel)), and naturalness for professional interpreting"
            exampleSentenceRule = "ALL example sentences (correction, next_level_alt, target_level_alt) MUST be COMPLETE SENTENCES in English (the practice language). Do NOT return Spanish or French for those fields. Do NOT return short labels — return full, natural sentences."
        } else {
            evaluateFocus = "verb tense and mood, gender/number agreement, register (\(registerLabel)), Anglicisms, and naturalness for professional interpreting"
